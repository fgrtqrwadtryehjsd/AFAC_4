"""V9 流水线 — 语义向量融合检索 Agent

核心突破：
1. DashScope text-embedding-v3 语义检索（不计入比赛Token预算！）
...
from agent.reasoner_v9 import ReasoningAgentV9
...
def run_a_board():
    print("=" * 60)
    print("AFAC2026 赛题四 - A 榜评测 V9")
    print("语义向量融合检索 Agent (Semantic-Vector Fusion Retrieval)")
    print("突破: Embedding检索(0Token) + BM25+Vector+Keyword三路RRF融合")
    ...
    qwen = QwenClient()
    agent = ReasoningAgentV9(qwen, doc_index, vector_indexer, token_budget=TOKEN_BUDGET)

    print(f"\n🧠 开始推理 (语义向量+BM25+关键词 三路融合)...")
    print("=" * 60)
    ...
    for i, q in enumerate(questions):
        ...
        ev_chars = trail.get("evidence_chars", 0)
        total_doc = trail.get("total_doc_chars", 0)
