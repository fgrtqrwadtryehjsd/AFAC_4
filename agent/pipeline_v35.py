"""V35 流水线: V31 + multi 证据重构（锚词前置 + 上限60K）"""
import os, json
...
def run_a_board():
    print("=" * 60)
    print("AFAC2026 V35 — multi 证据重构（锚词前置+60K）")
    print("=" * 60)
    ...
    qwen = QwenClient()
    agent = ReasoningAgentV35(qwen, di, None, token_budget=TOKEN_BUDGET)
    ...
    for i, q in enumerate(questions):
        ...
        strategy = "v35" if q.get("answer_format") == "multi" else "v31"
        print(f' {q["qid"]} ({q.get("domain","")[:4]}/{q.get("answer_format","")}) → {r["answer"]} [{strategy}|ev{r["evidence_chars"]//1000}K/{r["total_doc_chars"]//1000}K]')
    ...
    import shutil
    v35_csv = os.path.join(RESULTS_DIR, "answer_v35.csv")
    shutil.copy(out, v35_csv)
    print(f"  备份 V35 → {v35_csv}")
    ...
    print(f"\n  📊 V31参照: Score=48.50, Token=3.19M, TS=0.362")
    print(f"  ✅ 结果: {out}")
