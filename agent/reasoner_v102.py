"""V10.2: Verification Agent — 选项洗牌版

V10.1问题: 统一Prompt中选项ABCD固定顺序 → A=59 (position bias回归)
V10.2方案: 随机洗牌选项呈现顺序 + 输出时还原为ABCD

核心策略:
1. 每次调用随机排列选项在Prompt中的顺序（如 D→B→A→C）
2. 模型先看到D的证据，最后看到C的证据 → 打破A优先偏见
3. 模型输出时用洗牌后的顺序标注（如选第2个=D, 第3个=B）
4. 后处理将洗牌位置映射回原始ABCD标签
5. 保留V10.1的所有Token优化

V10.2期望: A占比下降到~25%（接近均匀分布），准确率不变
"""
import os
import json
import re
...
# 模板中用 {shuffled_options} 代替固定 ABCD 顺序
SHUFFLED_VERIFY_PROMPT = """## 任务
你是证据审查员。严格依据文档证据，对每个选项独立判断是否有明确原文支持。
注意：选项以随机顺序呈现，请勿受位置影响。
...
{option_review_sections}

最终答案：{answer_hint}

重要：请用原始选项标签（A/B/C/D）作答，不要用位置编号。"""
...
# ============ V10.2 Verification Agent (选项洗牌) ============

class ReasoningAgentV102:
    """V10.2: 逐选项检索 + 1次统一验证 + 选项随机洗牌

    V10: 4次独立验证 → A=26 (好) 但4.66M Token ❌
    V10.1: 1次统一验证 → 1.90M Token ✅ 但A=59 (position bias回归) ❌
    V10.2: 1次统一验证 + 选项洗牌 → 1.90M Token + A≈25 (均匀) ✅
    """
    ...
    def answer_question(self, question: dict) -> dict:
        ...
        shuffled_options_text = "\n".join(
            f"{label}. {options[label]}" for label in shuffled_labels)
        ...
        for label in shuffled_labels:
            ...
            review_sections.append(
                f"**选项{label}**：{opt_text}\n"
                f"原文引用：\n"
                f"分析：原文是否直接支持选项{label}的完整表述？\n"
                f"判断：✅ 明确支持 / ❌ 无明确支持 / ⚠️ 部分支持但不完全"
            )
        ...
        # Step 3: 1次统一CoT验证（洗牌顺序）
        system_prompt = DOMAIN_SYSTEM_PROMPTS.get(domain, "")
        prompt = SHUFFLED_VERIFY_PROMPT.format(
            evidence=evidence_text,
            question=q_text,
            shuffled_options=shuffled_options_text,
            option_review_sections=option_review_sections,
        ...
        self.cot_trails.append({
            ...
            "shuffled_order": "".join(shuffled_labels),
        })
