"""V15 流水线 — 两阶段 Locate-then-Verify

V15 = V13融合证据架构
    + Stage1 条款定位（Qwen轻量调用）
    + 增强 L1 压缩（去页眉页脚）
    + 带证据 Self-Critique（多选题）
    + 空答案补救扫描
    + faiss-cpu 向量检索
"""
...
from agent.reasoner_v15 import ReasoningAgentV15
...
def run_a_board():
    print("=" * 60)
    print("AFAC2026 赛题四 - A 榜评测 V15")
    print("两阶段 Locate-then-Verify + 增强L1压缩 + 带证据Self-Critique")
    ...
    qwen = QwenClient()
    agent = ReasoningAgentV15(qwen, doc_index, vector_indexer, token_budget=TOKEN_BUDGET)
    ...
    for i, q in enumerate(questions):
        ...
        threshold = result.get("full_doc_threshold", agent.memory.doc_index if False else 50000)
        is_full = result.get("is_full_doc", total_doc <= threshold)
        tag = f"全文<={threshold//1000}K" if is_full else f"检索>{threshold//1000}K"
        print(
    ...
    print(f"\n  📊 V13参照: Score=39.12 | TokenScore=0.630 | A=39 B=13 C=7 D=5")
