"""V31 流水线: 动态证据 — tf/mcq 精炼 + multi 完整"""
import os, json
...
def run_a_board():
    print("=" * 60)
    print("AFAC2026 V31 — 动态证据: tf/mcq精炼 + multi完整")
    print("=" * 60)
    ...
    import shutil
    v31_csv = os.path.join(RESULTS_DIR, "answer_v31.csv")
    shutil.copy(out, v31_csv)
    print(f"  备份 V31 → {v31_csv}")
    ...
    multi_ans = [r["answer"] for r in results
                 if r["answer"] and len(r["answer"]) == 1]
    ...
    print(f"\n  📊 参照:")
    print(f"    V22: Token=4.25M, TS=0.149, Score=44.69")
    print(f"    V30: Token=1.03M, TS=0.795, Score=34.72 (multi崩溃)")
    print(f"    V31预期: Token~3.3M, TS~0.345, Score~48.2")
