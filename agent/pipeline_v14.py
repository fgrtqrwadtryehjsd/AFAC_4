...
from agent.reasoner_v14 import ReasoningAgentV14
...
def run_a_board():
    print("=" * 60)
    print("AFAC2026 赛题四 - A 榜评测 V14")
    print("平衡检索与自适应证据架构")
    print("V13融合证据池 + 题型阈值 + 选项平衡补证据")
    ...
    qwen = QwenClient()
    agent = ReasoningAgentV14(qwen, doc_index, vector_indexer, token_budget=TOKEN_BUDGET)

    print(f"\n🧠 开始推理 (融合证据+RRF4路+选项补证据)...")
    print("=" * 60)
    ...
    for i, q in enumerate(questions):
        ...
        threshold = result.get("full_doc_threshold", 0)
        is_full = total_doc <= threshold
        tag = f"全文<={threshold//1000}K" if is_full else f"融合检索>{threshold//1000}K"
        print(
    ...
    if vector_indexer:
        ...
        print(
            f"  Embedding缓存: API{cache_stats['api_calls']}次, "
    ...
    print(f"\n  答案分布: {answer_dist}")
    print(
        f"  单选分布: A={single_dist.get('A',0)} B={single_dist.get('B',0)} "
