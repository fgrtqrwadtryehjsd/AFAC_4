...
V12 = V10.1核心 + 全文100K + 数值精准定位 + 条款3000字符 + 增强领域Prompt
不使用重标签(无效) 不使用强制评分(增加偏见) 不使用全题型Self-Critique(费Token)
"""
import os
import json
...
from agent.reasoner_v12 import ReasoningAgentV12
...
def run_a_board():
    print("=" * 60)
    print("AFAC2026 赛题四 - A 榜评测 V12")
    print("回归V10.1有效架构 + 证据优化")
    print("全文100K + 数值精准定位 + 条款3K上下文")
    ...
    qwen = QwenClient()
    agent = ReasoningAgentV12(qwen, doc_index, vector_indexer, token_budget=TOKEN_BUDGET)
